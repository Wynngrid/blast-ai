...
must_haves:
  truths:
    ...
    - "Session durations (20/40/60/90 min) map to slot counts per D-05"
  artifacts:
    ...
    - path: "src/lib/utils/timezone.ts"
      provides: "Timezone conversion utilities and session slot mapping"
  ...
  key_links:
    ...
    - from: "src/lib/utils/timezone.ts"
      to: "src/lib/constants/specializations.ts"
      via: "SESSION_DURATIONS and SLOT_CONFIG imports"
      pattern: "SLOT_CONFIG"
---

<objective>
Build the availability management system with FullCalendar, recurring weekly rules (per D-04), exception dates for blocking (per AVAIL-02), timezone-safe storage (per AVAIL-03), and session duration mapping (per D-05).

Purpose: Enable practitioners to set when they're available for sessions.
Output: Availability calendar UI with CRUD operations, timezone handling, and session type support.
</objective>
...
<task type="auto">
  <name>Task 1: Create availability validation schemas and timezone utilities with session mapping</name>
  ...
  <read_first>
    ...
    - src/lib/constants/specializations.ts (SLOT_CONFIG, SESSION_DURATIONS)
...
```
  ...
  <verify>
    <automated>grep -q "availabilityRuleSchema" src/lib/validations/availability.ts && grep -q "formatInUserTimezone" src/lib/utils/timezone.ts && grep -q "calculateBookableSlots" src/lib/utils/timezone.ts && echo "Schemas and utils OK"</automated>
  </verify>
  <done>Availability validation schemas and timezone utilities with session duration mapping created</done>
</task>
...
<task type="auto">
  <name>Task 3: Create availability calendar UI with FullCalendar and session info</name>
  ...
  <read_first>
    ...
    - src/lib/constants/specializations.ts (SESSION_DURATIONS, SLOT_CONFIG)
...
```
  ...
  <verify>
    <automated>grep -q "AvailabilityCalendar" src/components/portal/availability-calendar.tsx && grep -q "FullCalendar" src/components/portal/availability-calendar.tsx && grep -q "SESSION_DURATIONS" src/components/portal/availability-calendar.tsx && echo "Calendar OK"</automated>
  </verify>
  <done>FullCalendar availability management UI with session duration info created</done>
</task>
...
# Verify timezone utils with session mapping
grep "formatInUserTimezone" src/lib/utils/timezone.ts
grep "calculateBookableSlots" src/lib/utils/timezone.ts
...
# Verify calendar component with session types
grep "FullCalendar" src/components/portal/availability-calendar.tsx
grep "SESSION_DURATIONS" src/components/portal/availability-calendar.tsx
...
- Session duration mapping functions work per D-05 (20/40/60/90 min)
- Buffer calculation works per D-06 (15 min between sessions)
...
- UI shows session type info and which durations fit in time windows
